...
def get_portfolio_ids():
    """
    Returns Brand IDs from Asignacion Junio ONLY.
    Growth OS is not a numeric-data source — it is used only for email/phone.
    The portfolio (my brands) is defined exclusively by the Asignacion sheet.
    """
    ids = set()
    try:
        aj = load_asignacion_activa()
        if not aj.empty:
            ids.update(aj["brand_id"].apply(normalize_brand_id).dropna().astype(str))
    except Exception:
        pass
    ids.discard("")
    return ids
...
def get_current_ads_totals():
    ...
    REVENUE NET de la hoja Current ADS (fila Total excluida para no duplicar).
    Current ADS ya viene scoped al portafolio del farmer en el export de Rappi,
    así que se suma la hoja completa, no un sub-filtro.
    """
    try:
        raw = pd.read_excel(_excel_handle(EXCEL_FILE, _excel_mtime()), sheet_name=CURRENT_ADS_SHEET, header=0)
    except Exception:
        raw = pd.DataFrame()

    if raw.empty:
        return {"bookings_usd": 0, "revenue_usd": 0, "projected_revenue_usd": 0, "sales_usd": 0, "roi": 0}

    cols = {normalize(c).replace("_", " ").strip(): c for c in raw.columns}
    # Drop the machine "Total" row so we don't double-count.
    mask_total = raw.apply(lambda r: any(str(v).strip().lower() == "total" for v in r.values), axis=1)
    body = raw[~mask_total]

    def _sum(name):
        c = cols.get(name)
        return float(pd.to_numeric(body[c], errors="coerce").fillna(0).sum()) if c else 0.0

    bookings = _sum("bookings net")
...
def get_current_md_totals(pro=False):
    ...
    # Sales MD (MTD) = fila Total de la hoja (Current MD / Current MD pro), que es
    # el número oficial de la máquina. Se lee directo para no depender del filtro.
    sheet = CURRENT_MD_PRO_SHEET if pro else CURRENT_MD_SHEET
    sales_total_row = 0.0
    try:
        raw = pd.read_excel(_excel_handle(EXCEL_FILE, _excel_mtime()), sheet_name=sheet, header=0)
        cols = {normalize(c).replace("_", " ").strip(): c for c in raw.columns}
        sales_c = cols.get("sales md prime") if pro else cols.get("sales md $")
        mask_total = raw.apply(lambda r: any(str(v).strip().lower() == "total" for v in r.values), axis=1)
        if sales_c and mask_total.any():
            sales_total_row = float(pd.to_numeric(raw[mask_total][sales_c], errors="coerce").fillna(0).iloc[0])
    except Exception:
        sales_total_row = 0.0

    if df.empty:
        return {
            "sales_usd": sales_total_row,
            "gmv_usd": 0,
            "campaigns": 0,
            "orders": 0,
            "roi": 0,
    ...
    sales_rows = pd.to_numeric(df["_sales_usd"], errors="coerce").fillna(0).sum()
    sales = sales_total_row if sales_total_row > 0 else sales_rows
...
def _parse_excel_date_col(series):
    ...
    # Caso 3: ya viene como datetime.
    # Export ANTIGUO: openpyxl lo leía con formato m/d/yyyy y día/mes quedaban
    # intercambiados (1-jul se leía como 7-ene). Export NUEVO: la columna ya llega
    # correcta. Aplicar el swap a ciegas corrompía el archivo nuevo (todo julio se
    # dispersaba en ene/feb/…/ago). Detectamos si la columna YA es coherente y, en
    # ese caso, NO tocamos nada.
    s = pd.to_datetime(s, errors="coerce")

    valid = s.dropna()
    if not valid.empty:
        # Coherencia: si las fechas tal cual ya se agrupan en ≤2 meses distintos,
        # la columna está bien (un mes de gestión abarca a lo sumo 2 meses ISO por
        # los bordes de semana). Si están dispersas en 3+ meses, hay inversión real.
        months_present = valid.dt.to_period("M").nunique()
        if months_present <= 2:
            return s
...
def page_management_dashboard():
    ...
    # Actual MTD = Detalle CABA filtrado por portafolio (Asignacion), sumando
    # todas las stores por marca. Es la fuente oficial por decisión de negocio.
    # Fallback a Current GMV / Total row solo si Detalle CABA no está disponible.
    current_vals = get_portfolio_gmv_aov_from_detalle_caba()
    if not current_vals or current_vals.get("gmv_ars", 0) == 0:
        current_vals = get_current_gmv_aov_from_current_sheet() or get_current_gmv_totals() or {}
...
def _quick_goto_finder(df, key_prefix, id_col_candidates=("Brand ID", "brand_id", "ID", "Store ID"),
    ...
    para saltar al Brand Finder con la marca cargada.
